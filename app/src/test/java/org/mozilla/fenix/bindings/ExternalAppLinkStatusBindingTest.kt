...
class ExternalAppLinkStatusBindingTest {
    ...
    @Test
    fun `GIVEN the current url has no external app available WHEN a different url with an external app is selected THEN add open in app menu notification`() =
        runTest(testDispatcher) {
            ...
            browserStore.dispatch(TabListAction.SelectTabAction(tabId = "nonExternal"))
            testDispatcher.scheduler.advanceUntilIdle()

            verify {
                appStore.dispatch(AppAction.MenuNotification.AddMenuNotification(SupportedMenuNotifications.OpenInApp))
            }
        }

    @Test
    fun `GIVEN the current url has an external app available WHEN a different url without an external app is selected THEN remove open in app menu notification`() =
        runTest(testDispatcher) {
            val tabExternal = createTab(url = urlExternal, id = "external")
            val tabNonExternal = createTab(url = urlNonExternal, id = "nonExternal")
            val browserStore =
            ...
            browserStore.dispatch(TabListAction.SelectTabAction(tabId = "external"))
            testDispatcher.scheduler.advanceUntilIdle()
