...
class DefaultTabGroupRepositoryTest {
    ...
    fun `WHEN a tab group is created THEN add the entry to the database`() = runTest {
        ...
        assertEquals(
            expectedTabGroup,
            repository.tabGroupDataFlow.first().tabGroups.find { it.id == expectedTabGroup.id },
        )
    }
    ...
    @Test
    fun `WHEN a user closes an open tab group THEN mark the group as closed in the database and update its timestamp`() =
        runTest {
            val initialTabGroup =
                TabGroup(
                    title = "title",
    ...
    @Test
    fun `WHEN a user opens a closed tab group THEN mark the group as open in the database and update its timestamp`() =
        runTest {
            val initialTabGroup =
                TabGroup(
                    title = "title",
    ...
    @Test
    fun `WHEN a user closes all tab groups THEN mark all group as closed in the database and updated the affected groups' timestamps`() =
        runTest {
            val openTabGroups =
                List(size = 10) {
                    TabGroup(
            ...
            val expectedTabGroups =
                openTabGroups.map { it.copy(closed = true, lastModified = expectedTimestamp) } + alreadyClosedTabGroups
            initializeDatabase(initialTabGroups = openTabGroups + alreadyClosedTabGroups)
    ...
    @Test
    fun `WHEN a tab group assignment is passed-in THEN add the entry to the database and update the group's timestamp`() =
        runTest {
            val group =
                TabGroup(
                    title = "title",
            ...
            timeStamp = 7L
            initializeDatabase(initialTabGroups = listOf(group))
    ...
    @Test
    fun `WHEN a tab group assignment is created THEN add the entry to the database and update the group's timestamp`() =
        runTest {
            val group =
                TabGroup(
                    title = "title",
    ...
    @Test
    fun `WHEN a tab group assignment update is received THEN update the entry in the database and update the group's timestamp`() =
        runTest {
            val group =
                TabGroup(
                    title = "title",
    ...
    @Test
    fun `WHEN multiple tabs are assigned to a group THEN add the assignments to the database and update the group's timestamp`() =
        runTest {
            val group =
                TabGroup(
                    title = "title",
    ...
    @Test
    fun `WHEN a tab group assignment is deleted THEN remove the entry from the database and update the group's timestamp`() =
        runTest {
            val group =
                TabGroup(
                    title = "title",
    ...
    @Test
    fun `WHEN a tab group assignment is deleted via ID THEN remove the matching entry from the database and update the group's timestamp`() =
        runTest {
            val group =
                TabGroup(
                    title = "title",
    ...
    @Test
    fun `WHEN multiple tab group assignments are deleted THEN remove the entries from the database and update the groups' timestamps`() =
        runTest {
            val group1 =
                TabGroup(
                    title = "title",
    ...
    @Test
    fun `WHEN a tab group's tabs are all unassigned THEN remove the assignments from the database and update the group's timestamp`() =
        runTest {
            val group =
                TabGroup(
                    title = "title",
    ...
    @Test
    fun `WHEN a tab group is deleted via ID THEN the matching entry and its associated assignments are removed from the database`() =
        runTest {
            val tabGroupId = "1"
            val tabGroup1 =
                TabGroup(
    ...
    @Test
    fun `WHEN a subset of tab groups are deleted via ID THEN all of the matching entries and their associated assignments are removed from the database`() =
        runTest {
            val tabGroup1 =
                TabGroup(
                    title = "title",
