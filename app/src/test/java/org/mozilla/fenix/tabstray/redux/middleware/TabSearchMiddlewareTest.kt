...
class TabSearchMiddlewareTest {
    ...
    @Test
    fun `WHEN SearchQueryChanged on NormalTabs THEN search results include matching tabs inside tab groups`() =
        runTest {
                ...
                TabsTrayStore(
                    ...
                    initialState =
                        TabsTrayState(
                            ...
                            normalTabsState = TabsTrayState.NormalTabsState(items = listOf(tabGroup, otherNormalTab)),
                        ),
                )
    ...
    @Test
    fun `WHEN SearchQueryChanged on NormalTabs THEN search results include matching normal and inactive tabs`() =
        runTest {
            val expectedNormalTabs =
                listOf(
                ...
                TabsTrayStore(
                    ...
                    initialState =
                        TabsTrayState(
                            selectedPage = Page.NormalTabs,
                            normalTabsState =
                                TabsTrayState.NormalTabsState(items = expectedNormalTabs + otherNormalTabs),
                            inactiveTabs =
                                TabsTrayState.InactiveTabsState(tabs = expectedInactiveTabs + otherInactiveTabs),
                        ),
                )
            ...
            TabsTrayStore(
                ...
                initialState =
                    TabsTrayState(
                        ...
                        privateBrowsing =
                            TabsTrayState.PrivateBrowsingState(tabs = expectedPrivateTabs + otherPrivateTabs),
                    ),
            )
    ...
    @Test
    fun `WHEN SearchQueryChanged with multiple words in query on NormalTabs THEN search results include matching normal and inactive tabs`() =
        runTest {
            val expectedNormalTabs =
                listOf(TabsTrayItem.Tab(tab = createTab(url = "mozilla.org", title = "Mozilla Homepage")))
            val otherNormalTabs =
                listOf(
            ...
            val expectedInactiveTabs =
                listOf(
                    TabsTrayItem.Tab(tab = createTab(url = "support.mozilla.org", title = "Mozilla Homepage - Support"))
                )
            val otherInactiveTabs =
                listOf(TabsTrayItem.Tab(tab = createTab(url = "inactive.com", title = "example 3 title")))
    ...
    @Test
    fun `WHEN SearchQueryChanged with multiple words in query on PrivateTabs THEN search results include matching private tabs only`() =
        runTest {
            val expectedPrivateTabs =
                listOf(
                    ...
                    TabsTrayItem.Tab(
                        tab =
                            createTab(url = "support.mozilla.org", title = "Mozilla Homepage - Support", private = true)
                    ),
                )
            val otherPrivateTabs =
                listOf(
    ...
    @Test
    fun `WHEN SearchQueryChanged on NormalTabs THEN search results match query in title for both normal and inactive tabs`() =
        runTest {
            val expectedNormalTabs =
                listOf(TabsTrayItem.Tab(tab = createTab(url = "example.com", title = "Mozilla Homepage")))
            val otherNormalTabs =
                listOf(TabsTrayItem.Tab(tab = createTab(url = "example2.com", title = "Unrelated title")))
            val expectedInactiveTabs =
                listOf(TabsTrayItem.Tab(tab = createTab(url = "inactive-example.com", title = "Mozilla Inactive Tab")))
            val otherInactiveTabs =
                listOf(TabsTrayItem.Tab(tab = createTab(url = "inactive-example2.com", title = "Another title")))
    ...
    @Test
    fun `WHEN SearchQueryChanged on PrivateTabs THEN search results match query in title even if url does not contain it`() =
        runTest {
            val expectedPrivateTabs =
                listOf(
                    TabsTrayItem.Tab(tab = createTab(url = "mozilla.org", title = "Mozilla Homepage", private = true))
            ...
            val otherPrivateTabs =
                listOf(TabsTrayItem.Tab(tab = createTab(url = "example.com", title = "example title", private = true)))
        ...
        val store =
            TabsTrayStore(
                ...
                initialState =
                    TabsTrayState(
                        selectedPage = Page.NormalTabs,
                        normalTabsState = TabsTrayState.NormalTabsState(items = normalTabs),
                        inactiveTabs = TabsTrayState.InactiveTabsState(tabs = inactiveTabs),
                    ),
            )
        ...
        val store =
            TabsTrayStore(
                ...
                initialState =
                    TabsTrayState(
                        selectedPage = Page.PrivateTabs,
                        privateBrowsing = TabsTrayState.PrivateBrowsingState(tabs = privateTabs),
                    ),
            )
    ...
    @Test
    fun `WHEN SearchQueryChanged AND multiple about home tabs match THEN only the most recently accessed about home tab is included`() =
        runTest {
            val aboutHomeOld =
                TabsTrayItem.Tab(tab = createTab(url = ABOUT_HOME_URL, title = "Homepage", lastAccess = 10L))
            val aboutHomeNew =
                TabsTrayItem.Tab(tab = createTab(url = ABOUT_HOME_URL, title = "Homepage", lastAccess = 20L))
            val aboutHomeNewest =
                TabsTrayItem.Tab(tab = createTab(url = ABOUT_HOME_URL, title = "Homepage", lastAccess = 30L))

            val matchingNonHomepage =
                listOf(
                    TabsTrayItem.Tab(tab = createTab(url = "mozilla.org/home")),
                    TabsTrayItem.Tab(tab = createTab(url = "mozilla.org", title = "Homepage")),
                )
            val nonMatching = listOf(TabsTrayItem.Tab(tab = createTab(url = "example.com")))
    ...
    @Test
    fun `WHEN SearchQueryChanged AND about home tabs do not match query THEN no about home tabs are included`() =
        runTest {
            val aboutHomeOld =
                TabsTrayItem.Tab(tab = createTab(url = ABOUT_HOME_URL, title = "Homepage", lastAccess = 10L))
            val aboutHomeNew =
    ...
    fun `WHEN SearchQueryChanged AND single about home tab matches THEN it is included`() = runTest {
        ...
        val matchingNonHomepage = listOf(TabsTrayItem.Tab(tab = createTab(url = "mozilla.org/home")))
            ...
            TabsTrayStore(
                ...
                initialState =
                    TabsTrayState(
                        selectedPage = Page.NormalTabs,
                        normalTabsState =
                            TabsTrayState.NormalTabsState(items = listOf(aboutHome) + matchingNonHomepage),
                        inactiveTabs = TabsTrayState.InactiveTabsState(tabs = emptyList()),
                    ),
            )
