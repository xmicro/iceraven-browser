...
class SyncedTabsViewErrorTypeTest {
    ...
    fun `GIVEN synced tabs error types WHEN the synced tabs update process errors THEN the correct error text should be displayed`() {
        ...
        val multipleDevicesUnavailable =
            SyncedTabsView.ErrorType.MULTIPLE_DEVICES_UNAVAILABLE.toSyncedTabsListItem(context, navController)
        val syncEngineUnavailable =
            SyncedTabsView.ErrorType.SYNC_ENGINE_UNAVAILABLE.toSyncedTabsListItem(context, navController)
        val syncNeedsReauthentication =
            SyncedTabsView.ErrorType.SYNC_NEEDS_REAUTHENTICATION.toSyncedTabsListItem(context, navController)
        ...
        assertEquals(
            testContext.getString(R.string.synced_tabs_connect_another_device),
            multipleDevicesUnavailable.errorText,
        )
