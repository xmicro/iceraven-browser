...
class RecentSyncedTabFeatureTest {
    ...
    fun setup() {
        ...
        every { anyConstructed<SyncEnginesStorage>().getStatus() } returns mapOf(SyncEngine.Tabs to true)
    ...
    @Test
    fun `GIVEN current tab state is none WHEN account becomes available THEN loading state is dispatched, devices are refreshed, and a sync is started`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `GIVEN current tab state is not none WHEN account becomes available THEN loading state is not dispatched`() =
        runTest(testDispatcher) {
            val account = mockk<Account>()
            syncStore.setState(account = account)
    ...
    @Test
    fun `GIVEN synced tabs WHEN status becomes idle THEN recent synced tab is dispatched`() =
        runTest(testDispatcher) {
            val account = mockk<Account>()
            syncStore.setState(account = account)
            every { appStore.state } returns
            ...
            coEvery { syncedTabsStorage.getSyncedDeviceTabs() } returns
                listOf(
                    SyncedDeviceTabs(
    ...
    @Test
    fun `GIVEN loading state has not been dispatched WHEN status becomes idle THEN timing distribution is not recorded`() =
        runTest(testDispatcher) {
            val account = mockk<Account>()
            syncStore.setState(account = account)
            every { appStore.state } returns
            ...
            assertEquals(
                0,
                RecentSyncedTabs.recentSyncedTabTimeToLoad.testGetNumRecordedErrors(ErrorType.INVALID_STATE),
            )
        }

    @Test
    fun `GIVEN tabs from remote and current devices WHEN dispatching recent synced tab THEN current device is filtered out of dispatch`() =
        runTest(testDispatcher) {
            val account = mockk<Account>()
            syncStore.setState(account = account)
            every { appStore.state } returns
            ...
            verify {
                appStore.dispatch(AppAction.RecentSyncedTabStateChange(RecentSyncedTabState.Success(expectedTabs)))
            }
        }

    @Test
    fun `GIVEN there are devices with empty tabs list WHEN dispatching recent synced tab THEN devices with empty tabs list are filtered out`() =
        runTest(testDispatcher) {
            val account = mockk<Account>()
            syncStore.setState(account = account)
            every { appStore.state } returns
    ...
    fun `GIVEN tabs from different remote devices WHEN dispatching recent synced tab THEN most recently accessed tabs are set in the Success state`() =
        runTest {
            ...
            val expectedTabs =
                (firstDeviceTabs + secondDeviceTabs)
    ...
    @Test
    fun `GIVEN sync tabs are disabled WHEN dispatching recent synced tab THEN dispatch none`() =
        runTest(testDispatcher) {
            val account = mockk<Account>()
            syncStore.setState(account = account)
            every { appStore.state } returns
            ...
            every { anyConstructed<SyncEnginesStorage>().getStatus() } returns mapOf(SyncEngine.Tabs to false)

            val firstTab = createActiveTab("remote", "https://mozilla.org", null)
            val syncedTabs = listOf(SyncedDeviceTabs(deviceAccessed1, listOf(firstTab)))
            coEvery { syncedTabsStorage.getSyncedDeviceTabs() } returns syncedTabs
    ...
    @Test
    fun `WHEN synced tab dispatched THEN labeled counter metric recorded with device type`() =
        runTest(testDispatcher) {
            val account = mockk<Account>()
            syncStore.setState(account = account)
            every { appStore.state } returns
    ...
    @Test
    fun `WHEN synced tab dispatched THEN load time metric recorded`() =
        runTest(testDispatcher) {
            val account = mockk<Account>()
            syncStore.setState(account = account)
            every { appStore.state } returns
    ...
    @Test
    fun `GIVEN that the dispatched tab was the last dispatched tab WHEN dispatched THEN recorded as stale`() =
        runTest(testDispatcher) {
            val account = mockk<Account>()
            syncStore.setState(account = account)
            every { appStore.state } returns
    ...
    @Test
    fun `GIVEN that the dispatched tab was not the last dispatched tab WHEN dispatched THEN not recorded as stale`() =
        runTest(testDispatcher) {
            val account = mockk<Account>()
            syncStore.setState(account = account)
            every { appStore.state } returns
    ...
    fun `GIVEN current tab state is loading WHEN error is observed THEN tab state is dispatched as none`() =
        runTest(testDispatcher) {
            ...
            every { appStore.state } returns
                mockk {
                    every { recentSyncedTabState } returnsMany
                        listOf(
    ...
    @Test
    fun `GIVEN current tab state is not loading WHEN error is observed THEN nothing is dispatched`() =
        runTest(testDispatcher) {
            feature.start()
            syncStore.setState(status = SyncStatus.Error)
            testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `GIVEN that a tab has been dispatched WHEN LoggedOut is observed THEN tab state is dispatched as none`() =
        runTest(testDispatcher) {
            val account = mockk<Account>()
            syncStore.setState(account = account)
            every { appStore.state } returns
            ...
            coEvery { syncedTabsStorage.getSyncedDeviceTabs() } returns
                listOf(SyncedDeviceTabs(deviceAccessed1, listOf(tab)))
    ...
    @Test
    fun `GIVEN history entry contains synced tab host and has a preview image URL WHEN dispatched THEN preview url is included`() =
        runTest(testDispatcher) {
            val account = mockk<Account>()
            syncStore.setState(account = account)
            every { appStore.state } returns
            ...
            coEvery { historyStorage.getDetailedVisits(any(), any()) } returns
                listOf(activeTab.toVisitInfo(longerThanSyncedTabUrl, previewUrl))
    ...
    @Test
    fun `GIVEN history entry contains synced tab host but has no preview image URL WHEN dispatched THEN preview url is not included`() =
        runTest(testDispatcher) {
            val account = mockk<Account>()
            syncStore.setState(account = account)
            every { appStore.state } returns
            ...
            val longerThanSyncedTabUrl = activeTab.active().url + "/some/more/paths"
            coEvery { historyStorage.getDetailedVisits(any(), any()) } returns
                listOf(activeTab.toVisitInfo(longerThanSyncedTabUrl, null))
    ...
    private fun Tab.toVisitInfo(url: String, previewUrl: String?) =
        VisitInfo(
            title = this.active().title,
            url = url,
            visitTime = 0L,
