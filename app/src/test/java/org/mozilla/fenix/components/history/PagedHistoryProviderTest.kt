...
class PagedHistoryProviderTest {
    ...
    fun `getHistory uses getVisitsPaginated`() = runTest {
        val provider = DefaultPagedHistoryProvider(historyStorage = storage)
    ...
    fun `redirects are filtered out from history metadata groups`() = runTest {
        ...
        val historyMetadataKey3 =
            HistoryMetadataKey("http://www.google.com/link?url=http://www.firefox.com", "mozilla", null)
        val historyEntry3 =
            HistoryMetadata(
                key = historyMetadataKey3,
                title = "",
        ...
        coEvery { storage.getHistoryMetadataSince(any()) } returns
            listOf(historyEntry1, historyEntry2, historyEntry3, historyEntry4)
