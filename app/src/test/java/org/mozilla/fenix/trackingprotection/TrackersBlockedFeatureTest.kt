...
class TrackersBlockedFeatureTest {
    ...
    @Test
    fun `GIVEN feature started WHEN there is no tracker blocked event for the current tab THEN fetch trackers blocked information`() =
    ...
    @Test
    fun `GIVEN feature started WHEN a tracker is blocked THEN fetch the total trackers blocked and trackers blocked this week but not earliest date also`() =
        runTest(testDispatcher) {
            startFeature()
            shadowOf(Looper.getMainLooper()).idle()
    ...
    @Test
    fun `GIVEN multiple trackers are blocked in quick succession THEN sync is debounced`() =
        runTest(testDispatcher) {
            startFeature()
            shadowOf(Looper.getMainLooper()).idle()
    ...
    @Test
    fun `GIVEN a successful response for fetching blocked tracker events THEN dispatch a category for each tracker category with summed counts`() =
        runTest(testDispatcher) {
            startFeature()
    ...
    @Test
    fun `GIVEN a successful response for fetching blocked tracker events THEN sum fingerprint events into the Fingerprinters bucket`() =
        runTest(testDispatcher) {
            startFeature()
            ...
            appActionsCaptorMiddleware.assertLastAction(UpdateTrackersBlockedThisWeek::class) { action ->
                val fingerprinters =
                    action.blockedTrackerCategories.single {
                        it.name == R.plurals.trackers_blocked_panel_num_fingerprinters
                    }
    ...
    @Test
    fun `GIVEN a successful response for fetching blocked tracker events WHEN some events are unrecognized THEN those are excluded`() =
        runTest(testDispatcher) {
            startFeature()
    ...
    @Test
    fun `GIVEN a successful response for fetching blocked tracker events WHEN the response is empty THEN update the state with no blocked tracker`() =
        runTest(testDispatcher) {
            startFeature()
    ...
    @Test
    fun `GIVEN a successful response for fetching the earliest date of blocked trackers THEN update the state with this date`() =
        runTest(testDispatcher) {
            startFeature()
