...
class TabbedBrowsingTest {
    ...
    fun closingPrivateTabsTest() {
        ...
        homeScreen(composeTestRule) {}.togglePrivateBrowsingMode(switchPBModeOn = true)
        navigationToolbar(composeTestRule) {}
            .enterURLAndEnterToBrowser(genericURL.url) {}
            .openTabDrawer(composeTestRule) {
                verifyExistingOpenTabs("Test_Page_1")
    ...
    @Converted(
        replacedBy =
            ["org.mozilla.fenix.ui.efficiency.tests.TabbedBrowsingTest#verifyCloseAllPrivateTabsNotificationTest"],
        bug = 2063303,
        since = "2026-08",
        notes =
            "Port asserts the erase-notification title only (not the body text); tightened the final check to the private homepage (PRIVATE_BROWSING_INFO_CARD_TITLE).",
    )
    @SmokeTest
    @Test
    ...
    @Converted(
        replacedBy =
            ["org.mozilla.fenix.ui.efficiency.tests.TabbedBrowsingTest#verifySyncedTabsWhenUserIsNotSignedInTest"],
        bug = 2039245,
        since = "2026-05",
    )
    @Test
    ...
    @Converted(
        replacedBy =
            ["org.mozilla.fenix.ui.efficiency.tests.TabbedBrowsingTest#privateTabsDoNotPersistAfterClosingAppTest"],
        bug = 2058121,
        since = "2026-07",
    )
    @SmokeTest
