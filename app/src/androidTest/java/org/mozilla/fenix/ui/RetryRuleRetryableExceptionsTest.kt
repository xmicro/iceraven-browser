...
@LargeTest
class RetryRuleRetryableExceptionsTest(private val case: ThrowableCase) {
    ...
    val retryableComposeTestRule = RetryableComposeTestRule {
        AndroidComposeTestRuleV2(HomeActivityIntentTestRule()) { it.activity }
    }
    ...
    companion object {
        ...
        fun data(): Collection<Array<Any>> =
            listOf(
                ...
                arrayOf(
                    ThrowableCase("IdlingResourceTimeoutException") {
                        IdlingResourceTimeoutException(listOf("Retryable IdlingResourceTimeoutException"))
                    }
                ),
                arrayOf(
                    ThrowableCase("RuntimeException") {
                        RuntimeException("Retryable RuntimeException")
                    }
                ),
                arrayOf(
                    ThrowableCase("NullPointerException") {
