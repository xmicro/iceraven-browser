...
class SettingsSearchTest {
    ...
    @Converted(
        replacedBy =
            ["org.mozilla.fenix.ui.efficiency.tests.SettingsSearchTest#verifyTheDefaultSearchEngineCanBeChangedTest"],
        bug = 2041713,
        since = "2026-05",
    )
    @SmokeTest
    ...
    fun verifyEnabledUrlAutocompleteToggleTest() {
        ...
        if (FxNimbus.features.suggestShippedDomains.value().enabled) {
            // If true it will use the hardcoded list of "top domain" suggestions for the address bar's autocomplete
            // suggestions
            homeScreen(composeTestRule) {}
                .openSearch {
                    typeSearch("mo")
            ...
            // The suggestions for the address bar's autocomplete will take use of the user's local browsing history and
            // bookmarks
            ...
            homeScreen(composeTestRule) {}
                .openSearch {
                    typeSearch("moz")
                    // "Top domain" suggestions from the address bar's autocomplete are disabled, "moz" shouldn't
                    // autocomplete to mozilla.org
    ...
    @Converted(
        replacedBy =
            [
                "org.mozilla.fenix.ui.efficiency.tests.SettingsSearchTest#verifyCustomSearchEngineCanBeAddedFromSearchEngineMenuTest"
            ],
        bug = 2062943,
        since = "2026-08",
        notes =
            "Legacy also asserted the default-engine radio is checked inline; covered indirectly by the ported Settings > Search summary and search-selector-icon assertions.",
    )
    @SmokeTest
    @Test
    ...
    fun verifyShowClipboardSuggestionsToggleTest() {
        ...
        homeScreen(composeTestRule) {}.openTabDrawer {}.openNewTab {}
        navigationToolbar(composeTestRule) {
    ...
    @Converted(
        replacedBy =
            [
                "org.mozilla.fenix.ui.efficiency.tests.SettingsSearchTest#verifySearchShortcutChangesAreReflectedInSearchSelectorMenuTest"
            ],
        bug = 2062943,
        since = "2026-08",
    )
