...
class VisibilityLifecycleCallback(private val activityManager: ActivityManager?) :
    ...
    /**
     * Finishes and removes the list of AppTasks only if the application is in the background. The application is
     * considered to be in the background if it has at least 1 Activity in the started state
     *
     * @return True if application is in background (also finishes and removes all AppTasks), false otherwise
     */
    ...
    /** For private tabs, set the tracking protection exception to the default state if the app is closed. */
    private fun removeTCPException(activity: Activity) {
        activity.components.core.store.state.privateTabs
            .filter {
    ...
    companion object {
         ...
         *
         * @return True if application is in background (and consequently, finishes and removes all tasks), false
         *   otherwise.
         */
        internal fun finishAndRemoveTaskIfInBackground(context: Context): Boolean {
            return (context.applicationContext as FenixApplication)
                ...
                ?.finishAndRemoveTaskIfInBackground() ?: false
        }
    }
}
