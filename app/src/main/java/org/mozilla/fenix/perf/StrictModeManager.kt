...
open class StrictModeManager(
    ...
    /**
     * The number of times StrictMode has been suppressed. StrictMode can be used to prevent main thread IO but it's
     * easy to suppress. We use this value, in combination with:
     ...
     * - code owners: to prevent modifications to these above items without perf knowing to make suppressions a more
     *   deliberate act.
     *
     * This is an Atomic* so it can be safely incremented from any thread.
     */
    @VisibleForTesting(otherwise = PRIVATE) val suppressionCount = AtomicLong(0)

    /**
     * Enables strict mode for debug purposes. Meant to be run only in the main process. For simplicity it is also only
     * supported on the main thread.
     *
    ...
    /**
     * Revert strict mode to disable penalty based on fragment lifecycle since strict mode needs to switch to penalty
     * logs. Using the fragment life cycle allows decoupling from any specific fragment.
     */
    ...
    /**
     * Runs the given [functionBlock] while temporarily suppressing StrictMode checks if they are enabled by using the
     * [allowFn] before the call. A Gecko Profiler marker is recorded even if the strict mode checks are disabled to
     * allow investigating performance of release builds.
     *
     * For convenience we allow calling this on non-main threads in which case the [functionBlock] is run wihtou
     * profiler markers or StrictMode effects.
     *
    ...
    /**
     * A [FragmentManager.FragmentLifecycleCallbacks] implementation that handles StrictMode's `penaltyDeath` for
     * fragments.
     *
     * Note that if we use anonymous classes/functions in this class, we get a class load error with a slight perf
     * impact. See https://github.com/mozilla-mobile/fenix/issues/18731 for details.
     */
...
/**
 * There are certain manufacturers that have custom font classes for the OS systems. These classes violates the
 * [StrictMode] policies on startup. As a workaround, we create an exception list for these manufacturers so that
 * dialogs do not show up on start up. To add a new manufacturer to the list, log "Build.MANUFACTURER" from the device
 * to get the exact name of the manufacturer.
 */
