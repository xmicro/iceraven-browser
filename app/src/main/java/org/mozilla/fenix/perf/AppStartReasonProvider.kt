 ...
 *   importance.
 */
class AppStartReasonProvider(private val processInfoProvider: ProcessInfoProvider) {
    ...
    enum class StartReason {
        ...
        /**
         * This [Application] instance was started due to a component that is not an Activity: this may include
         * Services, BroadcastReceivers, and ContentProviders. It may be possible to distinguish between these but it
         * hasn't been necessary to do so yet.
         */
        NON_ACTIVITY,
    }

    /**
     * The reason this [Application] instance was started. This will not be set immediately but is expected to be
     * available by the time the first frame is drawn for the foreground Activity.
     */
    var reason = StartReason.TO_BE_DETERMINED
        private set

    /** Registers the handlers needed by this class: this is expected to be called from [Application.onCreate]. */
