...
open class PerformanceInflater(
    ...
    /**
     * This code was taken from the PhoneLayoutInflater.java located in the android source code (Similarly,
     * AsyncLayoutInflater implements it the exact same way too which can be found in the Android Framework). This piece
     * of code was taken from the other inflaters implemented by Android since we do not want to change the inflater
     * behavior except to count the number of inflations that our app is doing for performance purposes. Looking at the
     * `super.OnCreateView(name, attrs)`, it hardcodes the prefix as "android.view." this means that a xml element such
     * as ImageButton will crash the app using android.view.ImageButton. This method only works with XML tag that
     * contains no prefix. This means that views such as androidx.recyclerview... will not work with this method.
     */
