...
/**
 * View-bound feature that retrieves a list of [HistoryHighlight]s and [HistoryMetadata] items which will be mapped to
 * [RecentlyVisitedItem]s and then dispatched to [AppStore] to be displayed on the home screen.
 *
 ...
 * @param scope The [CoroutineScope] used for IO operations related to querying history and then for dispatching
 *   updates.
...
class RecentVisitsFeature(
    ...
    internal fun updateState(
        ...
        appStore.dispatch(AppAction.RecentHistoryChange(getCombinedHistory(historyHighlights, historyGroups)))
    }

    /**
     * Get up to [MAX_RESULTS_TOTAL] items if available as an even split of history highlights and history groups. If
     * more items then needed are available then highlights will be more by one.
     *
    ...
    internal fun getCombinedHistory(
        ...
        val distinctHighlights = historyHighlights.removeHighlightsAlreadyInGroups(historyGroups)
        ...
        return if (totalItemsCount <= MAX_RESULTS_TOTAL) {
            ...
            while ((highlightCount + groupsCount) < MAX_RESULTS_TOTAL) {
                if (
                    (highlightCount + groupsCount) < MAX_RESULTS_TOTAL &&
                ...
                if (
                    (highlightCount + groupsCount) < MAX_RESULTS_TOTAL && historyGroups.getOrNull(groupsCount) != null
                ) {
                    groupsCount += 1
                }
            }
            ...
            getSortedHistory(
                distinctHighlights.sortedByDescending { it.lastAccessedTime }.take(highlightCount),
                historyGroups.sortedByDescending { it.lastAccessedTime }.take(groupsCount),
            )
        }
    }
     ...
     * - If a `highlight` maps to multiple metadata records, its lastAccessTime will be set to the most recently updated
     *   record.
     *
    ...
    internal fun getHistoryHighlights(
        ...
        return highlights.map {
            HistoryHighlightInternal(
                historyHighlight = it,
                lastAccessedTime =
                    highlightsLastUpdatedTime.firstOrNull { (url, _) -> url == it.url }?.second?.updatedAt ?: 0,
            )
        }
    }

    /**
     * Group all urls accessed following a particular search. Automatically dedupes identical urls and adds each url's
     * view time to the group's total.
     *
     * @param metadata List of history visits.
     * @return List of user searches and all urls accessed from those.
     */
    @VisibleForTesting
    internal fun getHistorySearchGroups(metadata: List<HistoryMetadata>): List<HistoryGroupInternal> {
        return metadata
    ...
    /**
     * Maps the internal highlights and search groups to the final objects to be returned. Items will be sorted by their
     * last accessed date so that the most recent will be first.
     */
    @VisibleForTesting
    internal fun getSortedHistory(
        ...
        return (historyHighlights + historyGroups)
            ...
            .map {
                when (it) {
                    is HistoryHighlightInternal ->
                        RecentHistoryHighlight(
                            title =
                                if (it.historyHighlight.title.isNullOrBlank()) {
