 ...
 *
 * When "Homepage as a tab" is enabled, Home represents a special tab (`about:home`). Swiping on the
 * Home toolbar should switch to adjacent tabs with a sliding preview, just like the browser does.
 * Because the Home toolbar and navigation bar are Compose composables rather than Android views,
 * their on-screen bounds are supplied through [toolbarLayoutRect] and [navBarLayoutRect] instead of
 * being read off a `View`. When the swipe lands on a non-home tab, this class navigates from
 * HomeFragment to BrowserFragment so the selected tab's content becomes visible.
 *
 ...
 * @param navBarLayoutRect Supplies the navigation bar's current bounds in screen coordinates, or
 * null if it is absent or has not been laid out yet.
 * @param toolbarLayoutRect Supplies the toolbar's current bounds in screen coordinates, or null if
...
class HomeSwipeIntegration(
    ...
    /**
     * Registers the toolbar swipe gesture handler on the home UI, enabling switching between
     * adjacent tabs. Callers are responsible for gating this on the relevant settings.
     */
