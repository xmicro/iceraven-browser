...
internal object SetupChecklistReducer {
    ...
    fun reduce(state: AppState, action: SetupChecklistAction): AppState =
        when (action) {
            is SetupChecklistAction.Init -> state
            is SetupChecklistAction.Closed ->
                state.copy(setupChecklistState = state.setupChecklistState?.copy(isVisible = false))
            ...
            is SetupChecklistAction.ChecklistItemClicked -> {
                ...
                if (clickedItem is ChecklistItem.Group && state.setupChecklistState != null) {
                    ...
                    val updatedGroups =
                        state.setupChecklistState.checklistItems.filterIsInstance<ChecklistItem.Group>().map {
                            existingGroup ->
                    ...
                    state.copy(
                        setupChecklistState =
                            state.setupChecklistState.copy(
            ...
            is SetupChecklistAction.TaskPreferenceUpdated -> {
                state.setupChecklistState?.let {
                    val updatedItems =
                        it.checklistItems.map { existingItem ->
                            ...
                            when (existingItem) {
                                ...
                                is ChecklistItem.Group -> {
                                    existingItem.copy(
                                        tasks =
                                            existingItem.tasks.map { existingTask ->
                                ...
                                is ChecklistItem.Task ->
                                    if (existingItem.type == updatedTaskType) {
