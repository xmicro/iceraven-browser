 ...
 *
 * @property isVisible Whether the setup checklist feature should be visible to the user. This is a temporary state to
 *   remove the setup view immediately without having to update the entire AppState. Visibility is handled by the
 *   preference once the app is restarted thereon.
...
fun getSetupChecklistTitle(context: Context, allTasksCompleted: Boolean) =
    with(context) {
...
private fun subtitleForGroups(context: Context, progress: Progress) =
    with(context) {
...
private fun Context.subtitleForTasks(progress: Progress) =
    when (progress.completedTasks) {
        NO_TASKS ->
            getString(
