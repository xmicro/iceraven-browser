...
sealed interface ReviewPromptAttemptResult {
    ...
    companion object {
        /**
         * The docs for [ReviewManager.launchReviewFlow] state 'In some circumstances the review flow will not be shown
         * to the user, e.g. they have already seen it recently, so do not assume that calling this method will always
         * display the review dialog.' However, investigation has shown that a [ReviewInfo] instance with the flag:
         * - 'isNoOp=true' indicates that the prompt has NOT been displayed.
         * - 'isNoOp=false' indicates that a prompt has been displayed. [ReviewManager.launchReviewFlow] will modify the
         *   ReviewInfo instance which can be used to determine which of these flags is present.
         *
         * The internals of ReviewInfo cannot be accessed directly or cast nicely, so let's simply use the object as a
         * string.
         */
