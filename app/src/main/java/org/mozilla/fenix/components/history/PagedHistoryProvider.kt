...
/**
 * Class representing a history entry. Contrast this with [History] that's the same, but with an assigned position, for
...
/** @param historyStorage An instance [PlacesHistoryStorage] that provides read/write methods for history data. */
class DefaultPagedHistoryProvider(private val historyStorage: PlacesHistoryStorage) : PagedHistoryProvider {

    /** Types of visits we currently do not display in the History UI. */
    private val excludedVisitTypes =
        listOf(
            VisitType.DOWNLOAD,
    ...
    /**
     * All types of visits that aren't redirects. This is used for fetching only redirecting visits from the store so
     * that we can filter them out.
     */
    private val notRedirectTypes =
    ...
    private suspend fun getHistoryAndSearchGroups(
        ...
        var history: List<HistoryDB.Regular> =
            historyStorage
                .getVisitsPaginated(
                    offset.toLong(),
