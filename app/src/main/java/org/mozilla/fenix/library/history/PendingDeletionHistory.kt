...
fun History.toPendingDeletionHistory(): PendingDeletionHistory {
    return when (this) {
        ...
        is History.Group ->
            PendingDeletionHistory.Group(
                visitedAt = visitedAt,
                historyMetadata =
                    items.map { historyMetadata ->
...
fun Set<History>.toPendingDeletionHistory(): Set<PendingDeletionHistory> = map { it.toPendingDeletionHistory() }.toSet()
