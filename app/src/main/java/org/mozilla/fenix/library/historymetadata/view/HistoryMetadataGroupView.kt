...
class HistoryMetadataGroupView(
    ...
    private val binding =
        ComponentHistoryMetadataGroupBinding.inflate(
            LayoutInflater.from(container.context),
            container,
            true,
        )

    private val historyMetadataGroupAdapter =
        HistoryMetadataGroupAdapter(interactor) { isEmpty ->
    ...
    init {
        binding.historyMetadataGroupList.apply {
            layoutManager =
                LinearLayoutManager(containerView.context).also {
    ...
    /** Updates the display of the history metadata items based on the given [HistoryMetadataGroupFragmentState]. */
    fun update(state: HistoryMetadataGroupFragmentState) {
        ...
        if (selectedHistoryItems.isEmpty()) {
            ...
            setUiForSelectingMode(context.getString(R.string.history_multi_select_title, selectedHistoryItems.size))
        }
    }
}
