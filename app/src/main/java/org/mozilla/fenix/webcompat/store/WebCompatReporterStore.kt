...
data class WebCompatReporterState(
    ...
    enum class BrokenSiteReason(@param:StringRes val displayStringId: Int) {
        NotSupported(displayStringId = R.string.webcompat_reporter_reason_notsupported_2),
        Load(displayStringId = R.string.webcompat_reporter_reason_load),
        Media(displayStringId = R.string.webcompat_reporter_reason_media2),
        DeceptiveSite(displayStringId = R.string.webcompat_reporter_reason_site_is_deceptive),
        Content(displayStringId = R.string.webcompat_reporter_reason_content2),
        Slow(displayStringId = R.string.webcompat_reporter_reason_slow2),
        Checkout(displayStringId = R.string.webcompat_reporter_reason_checkout),
        Account(displayStringId = R.string.webcompat_reporter_reason_account2),
        AdBlocker(displayStringId = R.string.webcompat_reporter_reason_turn_off_adblocker),
        Other(displayStringId = R.string.webcompat_reporter_reason_other),
    }
...
sealed class WebCompatReporterAction : Action {
     ...
     * @property previewReporterItems The list of items to be displayed in the WebCompat Report preview.
     */
    data class PreviewItemsUpdated(val previewReporterItems: List<PreviewReporterItem>) : WebCompatReporterAction()
...
private fun reduce(
    ...
    when (action) {
        ...
        is WebCompatReporterAction.ProblemDescriptionChanged ->
            state.copy(problemDescription = action.newProblemDescription)
        ...
        is WebCompatReporterAction.PreviewItemsUpdated -> state.copy(previewReporterItems = action.previewReporterItems)
        ...
        is WebCompatReporterAction.EditUrlChanged -> state.copy(editedUrl = action.newUrl)
        WebCompatReporterAction.EditUrlClicked ->
            state.copy(
                showEditUrlDialog = true,
                editedUrl = state.enteredUrl,
            )
        WebCompatReporterAction.DismissEditUrlDialog -> state.copy(showEditUrlDialog = false)
        WebCompatReporterAction.SaveEditedUrlClicked ->
            state.copy(
...
/**
 * A [Store] that holds the [WebCompatReporterState] for the WebCompat Reporter and reduces [WebCompatReporterAction]s
...
class WebCompatReporterStore(
    ...
    private val _navEvents = MutableSharedFlow<WebCompatReporterAction.NavigationAction>(extraBufferCapacity = 1)

    val navEvents: SharedFlow<WebCompatReporterAction.NavigationAction>
        get() = _navEvents.asSharedFlow()

    internal fun emitNavAction(action: WebCompatReporterAction.NavigationAction) = _navEvents.tryEmit(action)
}
