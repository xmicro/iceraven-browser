...
internal fun bookmarksReducer(state: BookmarksState, action: BookmarksAction) =
    when (action) {
        ...
        is FolderClicked ->
            when {
                state.selectedItems.isNotEmpty() && action.item.isDesktopFolder ->
                    state.copy(bookmarksSnackbarState = BookmarksSnackbarState.CantEditDesktopFolders)
        ...
        SignIntoSyncClicked,
        is ViewAppeared,
        PrivateBrowsingAuthorized -> state
    }
...
private fun BookmarksState.handleSearchAction(action: SearchAction): BookmarksState =
    when (action) {
        is SearchAction.SearchClicked ->
            copy(
        ...
        it.copy(
            expansionState =
                SelectFolderExpansionState.Open(
                    children = it.expansionState.children.updateItemInTree(guidToUpdate, transform)
                )
        )
    } else {
        it
...
private fun BookmarksState.handleSelectFolderAction(action: SelectFolderAction): BookmarksState {
    return when (action) {
        ...
        is SelectFolderAction.SearchClicked ->
            copy(bookmarksSelectFolderState = bookmarksSelectFolderState?.copy(isSearching = true))
        is SelectFolderAction.SearchDismissed ->
            copy(bookmarksSelectFolderState = bookmarksSelectFolderState?.copy(isSearching = false))
        ...
        is SelectFolderAction.ChevronClicked ->
            if (action.folder.expansionState is SelectFolderExpansionState.Open) {
                copy(
                    bookmarksSelectFolderState =
                        bookmarksSelectFolderState?.copy(
                            folders =
                                bookmarksSelectFolderState.folders.updateItemInTree(
        ...
        is SelectFolderAction.ExpandedFolderLoaded ->
            copy(
                bookmarksSelectFolderState =
                    bookmarksSelectFolderState?.copy(
                        folders =
                            bookmarksSelectFolderState.folders.updateItemInTree(action.folder.guid, { action.folder })
                    ) ?: bookmarksSelectFolderState
            )
...
private fun BookmarksState.handleEditBookmarkAction(action: EditBookmarkAction): BookmarksState {
    return when (action) {
        ...
        is EditBookmarkAction.TitleChanged ->
            this.copy(
                bookmarksEditBookmarkState =
                    bookmarksEditBookmarkState?.let {
                        it.copy(
...
private fun BookmarksState.handleAddFolderAction(action: AddFolderAction): BookmarksState {
    return when (action) {
        is AddFolderAction.ParentFolderClicked ->
            this.copy(
                bookmarksSelectFolderState =
                    bookmarksSelectFolderState?.copy(
        ...
        is AddFolderAction.FolderCreated ->
            this.copy(
                ...
                bookmarksEditBookmarkState = bookmarksEditBookmarkState?.copy(folder = action.folder),
            )
        is AddFolderAction.TitleChanged ->
            this.copy(
                bookmarksAddFolderState =
                    bookmarksAddFolderState?.copy(folderBeingAddedTitle = action.updatedText.replace("\n", " "))
            )
    }
}
...
private fun BookmarksState.handleEditFolderAction(action: EditFolderAction): BookmarksState {
    return when (action) {
        is EditFolderAction.TitleChanged ->
            this.copy(
                bookmarksEditFolderState =
                    bookmarksEditFolderState?.let {
                        it.copy(folder = it.folder.copy(title = action.updatedText.replace("\n", " ")))
                    }
            )

        is EditFolderAction.ParentFolderClicked ->
            this.copy(
                bookmarksSelectFolderState =
                    bookmarksSelectFolderState?.copy(
        ...
        is EditFolderAction.DeleteClicked ->
            bookmarksEditFolderState?.folder?.guid?.let {
                this.copy(
                    bookmarksDeletionDialogState =
                        DeletionDialogState.LoadingCount(listOf(bookmarksEditFolderState.folder.guid))
                )
            } ?: this
    }
}
...
private fun BookmarksState.handleDeletionDialogAction(action: DeletionDialogAction): BookmarksState {
    return when (action) {
        is DeletionDialogAction.CountLoaded ->
            this.copy(
                bookmarksDeletionDialogState =
                    DeletionDialogState.Presenting(
...
private fun BookmarksState.withDeletedItemsRemoved(): BookmarksState =
    when {
        bookmarksDeletionDialogState is DeletionDialogState.Presenting ->
            copy(
...
private fun BookmarksState.updateSelectedFolder(folder: SelectFolderItem): BookmarksState =
    when {
            ...
            if (bookmarksEditBookmarkState == null) {
                ...
                alwaysTryUpdate.copy(
                    ...
                    bookmarksSnackbarState =
                        BookmarksSnackbarState.BookmarkMoved(
...
internal fun formatBookmarkTitle(raw: String, max: Int = 25): String {
    val cleaned = raw.removePrefix("https://").removePrefix("http://").removePrefix("www.")
...
private fun BookmarksState.respondToBackClick(): BookmarksState =
    when {
        ...
        bookmarksSelectFolderState != null -> {
            when {
                bookmarksSelectFolderState.innerSelectionGuid != null -> {
                    copy(bookmarksSelectFolderState = bookmarksSelectFolderState.copy(innerSelectionGuid = null))
                }
...
private fun BookmarksState.handleSortMenuAction(action: BookmarksAction): BookmarksState =
    when (action) {
        ...
        SelectFolderAction.SortMenu.SortMenuButtonClicked -> copy(sortMenuShown = !sortMenuShown)
        BookmarksListMenuAction.SortMenu.SortMenuDismissed,
        SelectFolderAction.SortMenu.SortMenuDismissed -> copy(sortMenuShown = false)
        BookmarksListMenuAction.SortMenu.CustomSortClicked,
        SelectFolderAction.SortMenu.CustomSortClicked -> copy(sortOrder = BookmarksListSortOrder.Positional)
        BookmarksListMenuAction.SortMenu.NewestClicked,
        SelectFolderAction.SortMenu.NewestClicked -> copy(sortOrder = BookmarksListSortOrder.Created(true))
        BookmarksListMenuAction.SortMenu.OldestClicked,
        SelectFolderAction.SortMenu.OldestClicked -> copy(sortOrder = BookmarksListSortOrder.Created(false))
        BookmarksListMenuAction.SortMenu.AtoZClicked,
        SelectFolderAction.SortMenu.AtoZClicked -> copy(sortOrder = BookmarksListSortOrder.Alphabetical(true))

        BookmarksListMenuAction.SortMenu.ZtoAClicked,
        SelectFolderAction.SortMenu.ZtoAClicked -> copy(sortOrder = BookmarksListSortOrder.Alphabetical(false))
        else -> copy(sortOrder = BookmarksListSortOrder.Positional)
    }.let {
        it.copy(bookmarkItems = it.bookmarkItems.sortedWith(it.sortOrder.comparator))
    }
...
private fun BookmarksState.handleListMenuAction(action: BookmarksListMenuAction): BookmarksState =
    when (action) {
        ...
        is BookmarksListMenuAction.Bookmark.DeleteClicked ->
            copy(bookmarkItems = this.bookmarkItems.filterNot { it.guid == action.bookmark.guid })
        is BookmarksListMenuAction.Folder.DeleteClicked ->
            copy(bookmarksDeletionDialogState = DeletionDialogState.LoadingCount(listOf(action.folder.guid)))
        is BookmarksListMenuAction.Bookmark.MoveClicked ->
            copy(selectedItems = listOf(action.bookmark)).handleMoveClicked()
        is BookmarksListMenuAction.Folder.MoveClicked -> copy(selectedItems = listOf(action.folder)).handleMoveClicked()
...
private fun BookmarksState.handleMoveClicked(): BookmarksState =
    copy(
        bookmarksSelectFolderState = BookmarksSelectFolderState(outerSelectionGuid = currentFolder.guid),
