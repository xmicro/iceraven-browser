...
class DesktopFolders(
    ...
    suspend fun withOptionalDesktopFolders(node: BookmarkNode): BookmarkNode {
        return when (node.guid) {
            BookmarkRoot.Mobile.id -> {
                ...
                val childrenWithVirtualFolder = listOfNotNull(virtualDesktopFolder()) + node.children.orEmpty()
    ...
    suspend fun count(): Int {
        return bookmarksStorage
            .countBookmarksInTrees(listOf(BookmarkRoot.Menu.id, BookmarkRoot.Toolbar.id, BookmarkRoot.Unfiled.id))
            .toInt()
    }

    private suspend fun virtualDesktopFolder(): BookmarkNode? =
        bookmarksStorage.getTree(BookmarkRoot.Root.id, recursive = false).getOrNull()?.let {
    ...
    private fun restructureDesktopRoots(roots: List<BookmarkNode>?): List<BookmarkNode>? {
        ...
        return roots.filter { rootTitles.containsKey(it.title) }.map { it.copy(title = rootTitles[it.title]) }
    }

    /**
     * Restructures roots to place desktop roots underneath the mobile root and renames them to friendly titles. This
     * provides a recognizable bookmark tree when offering destinations to move a bookmark.
     */
    private fun restructureMobileRoots(roots: List<BookmarkNode>?): List<BookmarkNode>? {
        roots ?: return null

        val others = roots.filter { it.guid != BookmarkRoot.Mobile.id }.map { it.copy(title = rootTitles[it.title]) }
