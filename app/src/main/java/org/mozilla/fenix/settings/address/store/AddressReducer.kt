...
fun addressReducer(state: AddressState, action: AddressAction): AddressState {
    return when (action) {
        ...
        is AddressStructureLoaded ->
            state.copy(structureState = AddressStructureState.Loaded(action.structure)).update {
...
private fun UpdatableAddressFields.update(field: AddressStructure.Field.SelectField) =
    when (field.id) {
