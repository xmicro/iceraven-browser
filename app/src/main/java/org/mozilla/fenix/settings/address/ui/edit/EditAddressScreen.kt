...
fun EditAddressScreen(store: AddressStore) {
    Scaffold(
        ...
        LazyColumn(
            ...
            val firstTextField =
                structureState.structure.fields.firstOrNull {
                ...
                when (item) {
                    is AddressStructure.Field.TextField -> {
                        TextField(
                            ...
                            field = item,
                            modifier =
                                Modifier.thenConditional(Modifier.focusRequester(focusRequester)) {
                                    item == firstTextField
                                },
                        )
                    }
...
private fun TextField(
    ...
    val value by remember {
        store.stateFlow.map { it.address.valueForID(field.id) }
    }
...
private fun UpdatableAddressFields.valueForID(id: AddressStructure.Field.ID) =
    when (id) {
...
private fun AddressStructure.Field.ID.formChangeAction(value: String) =
    when (this) {
...
@Composable
private fun AddressStructure.Field.LocalizationKey.localizedString() =
    when (this) {
...
private fun createStore(
    ...
    AddressStore(
            initialState =
                AddressState.initial(region = region, address = address)
                    .copy(
                        structureState =
                            AddressStructureState.Loaded(
                                structure =
                                    AddressStructure(
                                        fields =
                                            listOf(
...
@Composable
private fun AddAddressPreview(@PreviewParameter(PreviewThemeProvider::class) theme: Theme) {
...
@FlexibleWindowPreview
@Composable
private fun EditAddressPreview(@PreviewParameter(PreviewThemeProvider::class) theme: Theme) {
    val store = createStore(address = generateAddress())
