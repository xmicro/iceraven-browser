...
/**
 * Helper for observing Translation state from both [BrowserState.translationEngine] and
...
class TranslationsDialogBinding(
            ...
            .collect { state ->
                ...
                translateFromLanguages?.let {
                    translationsDialogStore.dispatch(
                        TranslationsDialogAction.UpdateTranslateFromLanguages(translateFromLanguages)
                    )
                }
                ...
                translateToLanguages?.let {
                    translationsDialogStore.dispatch(
                        TranslationsDialogAction.UpdateTranslateToLanguages(translateToLanguages)
                    )
                }
                ...
                translationsDialogStore.dispatch(
                    TranslationsDialogAction.UpdateTranslationInProgress(sessionTranslationsState.isTranslateProcessing)
                )
                ...
                if (fromSelected != null && translationsDialogStore.state.initialFrom == null) {
                    translationsDialogStore.dispatch(TranslationsDialogAction.UpdateFromSelectedLanguage(fromSelected))
                }
                ...
                if (toSelected != null && translationsDialogStore.state.initialTo == null) {
                    translationsDialogStore.dispatch(TranslationsDialogAction.UpdateToSelectedLanguage(toSelected))
                }
                ...
                sessionTranslationsState.translationDownloadSize?.let {
                    translationsDialogStore.dispatch(TranslationsDialogAction.UpdateDownloadTranslationDownloadSize(it))
                }
    ...
    /**
     * Helper function for error handling, which requires considering both [TranslationsState] (session) and
     * [TranslationsBrowserState] (browser or global) states.
     *
     * Will dispatch [TranslationsDialogAction] to update store when appropriate.
     *
     * Certain errors take priority over others, depending on how important they are. Error priority:
     * 1. An error in [TranslationsBrowserState] of [EngineNotSupportedError] should be the highest priority to process.
     * 2. An error in [TranslationsState] of [LanguageNotSupportedError] is the second highest priority and requires
     *    additional information.
     * 3. Displayable browser errors only should be the dialog error when the session has a non-displayable error.
     *    (Usually expect this case where an error recovery action occurred on a different tab and failed.)
    ...
    private fun updateTranslationError(
        ...
        if (browserTranslationsState.engineError is TranslationError.EngineNotSupportedError) {
            ...
            val documentLangDisplayName =
                sessionTranslationsState.translationEngineState?.detectedLanguages?.documentLangTag?.let { docLangTag ->
            ...
            browserTranslationsState.engineError?.displayError == true &&
