...
/** Adapter for a list of sites that are exempted from saving logins, along with controls to remove the exception. */
class LoginExceptionsAdapter(interactor: LoginExceptionsInteractor) :
    ...
    override fun wrapAdapterItem(item: LoginException) = LoginAdapterItem(item)

    data class LoginAdapterItem(override val item: LoginException) : AdapterItem.Item<LoginException>() {
        override val url
            get() = item.origin
    }
