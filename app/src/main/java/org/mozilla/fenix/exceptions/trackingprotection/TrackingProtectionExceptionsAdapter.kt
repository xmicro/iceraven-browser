...
/**
 * Adapter for a list of sites that are exempted from Tracking Protection, along with controls to remove the exception.
 */
class TrackingProtectionExceptionsAdapter(interactor: TrackingProtectionExceptionsInteractor) :
    ...
    override fun wrapAdapterItem(item: TrackingProtectionException) = TrackingProtectionAdapterItem(item)

    data class TrackingProtectionAdapterItem(override val item: TrackingProtectionException) :
    ...
    internal object DiffCallback : DiffUtil.ItemCallback<AdapterItem>() {
        override fun areItemsTheSame(oldItem: AdapterItem, newItem: AdapterItem) =
            when (oldItem) {
                ...
                AdapterItem.Header -> oldItem === newItem
